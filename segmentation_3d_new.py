"""3D segmentation new (experimental).

Compared with segmentation_3d_last.py, this version uses foreground-aware
patch sampling and a slightly larger network to improve foreground learning.
"""
...
def make_sphere_volume(size: int, rng: np.random.Generator) -> tuple[np.ndarray, np.ndarray]:
    ...
    label[mask] = 1
    image[0] = rng.normal(0.0, 0.06, size=(size, size, size)).astype(np.float32)
    image[0][mask] += 1.0
    image[0] = np.clip(image[0], 0.0, 1.0)
    return image, label


def random_flip_3d(image: np.ndarray, label: np.ndarray, rng: np.random.Generator) -> tuple[np.ndarray, np.ndarray]:
    for axis in (1, 2, 3):
        if rng.random() < 0.5:
...
def intensity_jitter(image: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    scale = rng.uniform(0.9, 1.15)
    bias = rng.uniform(-0.05, 0.05)
    noise = rng.normal(0.0, 0.03, size=image.shape).astype(np.float32)
    image = image * scale + bias + noise
    return np.clip(image, 0.0, 1.0)


class Synthetic3DDataset(Dataset):
    def __init__(self, n: int, size: int, seed: int, strong_aug: bool = False) -> None:
        self.n = n
        self.size = size
        self.seed = seed
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if self.strong_aug:
            image, label = random_flip_3d(image, label, rng)
            image = intensity_jitter(image, rng)
        return {"image": torch.from_numpy(image), "label": torch.from_numpy(label)}


class ForegroundPatch3DDataset(Dataset):
    def __init__(self, n: int, full_size: int, patch_size: int, seed: int, fg_ratio: float = 0.7) -> None:
        self.n = n
    ...
    def _crop_patch(self, image: np.ndarray, label: np.ndarray, center: tuple[int, int, int]) -> tuple[np.ndarray, np.ndarray]:
        half = self.patch_size // 2
        size = self.full_size
        cz, cy, cx = center
        z0 = max(0, min(cz - half, size - self.patch_size))
        y0 = max(0, min(cy - half, size - self.patch_size))
        x0 = max(0, min(cx - half, size - self.patch_size))
        return (
            image[:, z0:z0 + self.patch_size, y0:y0 + self.patch_size, x0:x0 + self.patch_size],
            label[z0:z0 + self.patch_size, y0:y0 + self.patch_size, x0:x0 + self.patch_size],
        )

    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        rng = np.random.default_rng(self.seed + idx)
        image, label = make_sphere_volume(self.full_size, rng)
        fg_voxels = np.argwhere(label == 1)
        if len(fg_voxels) > 0 and rng.random() < self.fg_ratio:
            center = tuple(fg_voxels[rng.integers(0, len(fg_voxels))])
        else:
            center = (rng.integers(0, self.full_size), rng.integers(0, self.full_size), rng.integers(0, self.full_size))
        if rng.random() < 0.75:
            image, label = random_flip_3d(image, label, rng)
        image = intensity_jitter(image, rng)
        image, label = self._crop_patch(image, label, center)
        return {"image": torch.from_numpy(image), "label": torch.from_numpy(label)}


class TinyUNet3D(nn.Module):
    def __init__(self, in_channels: int = 1, out_channels: int = 2, base: int = 32) -> None:
        super().__init__()
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device) -> dict[str, float]:
    ...
    for batch in loader:
        ...
        total += 1
    return {"voxel_accuracy": total_acc / max(total, 1), "dice_fg": total_dice / max(total, 1)}
...
def train(args: argparse.Namespace) -> None:
    ...
    if args.quick:
        train_n, val_n, epochs = 24, 8, 8
    else:
        train_n, val_n, epochs = 64, 16, args.epochs

    train_ds = ForegroundPatch3DDataset(train_n, full_size=args.size, patch_size=args.patch_size, seed=args.seed, fg_ratio=args.fg_ratio)
    val_ds = Synthetic3DDataset(val_n, size=args.size, seed=args.seed + 10_000, strong_aug=False)
